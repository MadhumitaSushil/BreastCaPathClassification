...
def precision_recall_fscore_micro(gold, pred):
    ...
    p, r, f, _ = precision_recall_fscore_support(y_true=gold, y_pred=pred, average='micro', zero_division=False)
    return np.asarray([p, r, f])


def precision_recall_fscore_macro(gold, pred):
    """Get macro-averaged precision, recall, f-score."""
    p, r, f, _ = precision_recall_fscore_support(y_true=gold, y_pred=pred, average='macro', zero_division=False)
    return np.asarray([p, r, f])


def precision_recall_fscore_weighted(gold, pred):
    """Get weighted precision, recall, f-score."""
    p, r, f, _ = precision_recall_fscore_support(y_true=gold, y_pred=pred, average='weighted', zero_division=False)
    return np.asarray([p, r, f])
