...
from dataprocessor_transformers import PathProcessor, stopping_metrics
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.output_mode == 'multilabel_classification':
                ...
                logits = outputs[0]
                loss = loss_fn(logits, labels.to(logits))
            else:
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if args.output_mode == 'multilabel_classification':
                    ...
                    outputs = model(**inputs)
                    logits = outputs[0]
                    tmp_eval_loss = loss_fn(logits, labels.to(logits))
                else:
                    inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3]}
                    if args.model_type != "distilbert":
                        inputs["token_type_ids"] = (
                    ...
                    outputs = model(**inputs)
                    logits = outputs[1]
                    tmp_eval_loss = outputs[0]  # model outputs are always tuple in transformers (see doc)
        ...
        if args.output_mode == "classification":
            ...
            result = compute_metrics(preds, out_label_ids, examples, is_multilabel=False)
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)
            result = compute_metrics(preds, out_label_ids, examples, is_multilabel=False)
        elif args.output_mode == 'multilabel_classification':
            preds = torch.sigmoid(torch.tensor(preds)).numpy()
            preds = [[1 if p > 0.5 else 0 for p in pred] for pred in preds]
            result = compute_metrics(preds, out_label_ids, examples, is_multilabel=True)
        ...
        with open(output_eval_file.replace('_results.txt', '_predictions.tsv'), "w") as writer:
            for example, pred in zip(examples, preds):
                if args.output_mode == "classification":
                    ...
                    pred_label = pred# [args.label_list[ind] for ind, p in enumerate(pred) if p > 0.5]
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, return_examples=False):
    ...
    processor = PathProcessor(args.task_name, args.data_dir)
...
def main():
    ...
    parser.add_argument(
        "--model_name_or_path",
        default='/wynton/group/ichs/msushil/ucsf-bert/',
        type=str,
        required=False,
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if not args.hierarchical:
            ...
            model = HiBERT(args.output_dir, args.max_seq_length, args.decoder, num_labels, args.cache_dir)
        if 'roberta' in args.model_name_or_path:
            from transformers import RobertaTokenizerFast
            tokenizer = RobertaTokenizerFast.from_pretrained(
                args.output_dir,
    ...
    if (args.do_eval or args.do_test) and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if not args.hierarchical:
                ...
                model = HiBERT.from_pretrained(checkpoint, args.max_seq_length, args.decoder, num_labels,
